...
interface TextScriptEditorProps {
  ...
  onOptimizeText?: (goal: string) => Promise<void>;
          ...
          {onOptimizeText && (
            <div className="relative">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-300 rounded-lg border border-indigo-500/30 transition-colors disabled:opacity-40"
              >
                <Wand2 className={`w-3.5 h-3.5 ${isOptimizing ? "animate-spin" : ""}`} />
                <span>{isOptimizing ? "Polishing..." : "AI Enhance"}</span>
                <ChevronDown className="w-3 h-3 text-indigo-400/80" />
              </button>

              {showEnhancerMenu && (
                <div className="absolute right-0 top-full mt-1.5 w-60 bg-slate-900 border border-slate-700 rounded-xl shadow-2xl z-30 p-2 space-y-1">
                  <div className="px-2 py-1 text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">
                    Speech Script Goals
                  </div>
                  <button
                    type="button"
                    onClick={() => {
                      onOptimizeText("clarity");
                      setShowEnhancerMenu(false);
                    }}
                    className="w-full text-left px-2.5 py-2 rounded-lg hover:bg-slate-800 text-xs text-slate-200 flex flex-col"
                  >
                    <span className="font-semibold text-indigo-300">Natural Speech Cadence</span>
                    <span className="text-[10px] text-slate-400">Optimizes rhythm, breathing & pauses</span>
                  </button>
                  <button
                    type="button"
                    onClick={() => {
                      onOptimizeText("story");
                      setShowEnhancerMenu(false);
                    }}
                    className="w-full text-left px-2.5 py-2 rounded-lg hover:bg-slate-800 text-xs text-slate-200 flex flex-col"
                  >
                    <span className="font-semibold text-emerald-300">Audiobook Narrative</span>
                    <span className="text-[10px] text-slate-400">Vivid imagery and storytelling flow</span>
                  </button>
                  <button
                    type="button"
                    onClick={() => {
                      onOptimizeText("podcast");
                      setShowEnhancerMenu(false);
                    }}
                    className="w-full text-left px-2.5 py-2 rounded-lg hover:bg-slate-800 text-xs text-slate-200 flex flex-col"
                  >
                    <span className="font-semibold text-sky-300">Conversational Podcast</span>
                    <span className="text-[10px] text-slate-400">Upbeat hooks and engaging flow</span>
                  </button>
                </div>
              )}
            </div>
